...
class DAOFuncionarioUser extends GenericDAO {
    ...
    public function deleteByFuncionario($id = 0) {
        try {
            $this->query = "DELETE FROM funcionario_user WHERE fuus_fk_funcionario_pk_id=:fuus_fk_funcionario_pk_id;";
            $conexao = $this->getInstance();
            $this->statement = $conexao->prepare($this->query);
            $this->statement->bindParam(":fuus_fk_funcionario_pk_id", $id, PDO::PARAM_INT);
            $this->statement->execute();
        } catch (Exception $erro) {
            throw new Exception($erro->getMessage());
        }
    ...
    public function selectObjectById($fuus_pk_id = 0) {
        ...
        $this->query .= "INNER JOIN funcionario AS f ON (fp.fuus_fk_funcionario_pk_id=f.id) ";
    ...
    public function selectObjectsByContainsObject(ModelFuncionarioUser $funcionarioUser = null) {
        ...
        $this->query .= "* ";
        $this->query .= "FROM funcionario_user AS fu ";
        $this->query .= "INNER JOIN funcionario AS f ON (fu.fuus_fk_funcionario_pk_id=f.id) ";
        $this->query .= "INNER JOIN user AS u ON (fu.fuus_fk_id=u.id) ";
        $this->query .= "WHERE ";
        $this->query .= "f.nome LIKE '%$funcionarioUser->nome%' AND ";
        $this->query .= "f.cpf LIKE '%$funcionarioUser->cpf%' AND ";
    ...
    public function selectObjectByFkFuncionario($id = 0) {
        $this->query = "SELECT ";
        $this->query .= "* ";
        $this->query .= "FROM funcionario_user AS fu ";
        $this->query .= "INNER JOIN user AS u ON (fu.fuus_fk_id=u.id) ";
