...
$(document).ready(function () {
    ...
    $("select[name='permissao_id[]']").change(function(e){
        $("button[name='adicionar_permissao']").prop('disabled', false);
    });
...
<div class="row">
    ...
    <div class="col-lg-4 mb-4">
        <form action="<?php echo server_url('?page=ControllerGrupo&option=removerPermissoes'); ?>" method="post">
            <div class="card h-100">
                <h4 class="card-header text-primary">Desvincular Permissões de Grupo</h4>
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="permissao_id[]" class="form-control selectpicker" multiple data-actions-box="true">
                            <?php
                                foreach ($permissoesDoGrupo as $each_permissao) {
                                    echo '<option value="', $each_permissao->id, '"', ' selected>', $each_permissao->nome, '</option>';
                                }
                            ?>    
                    </select>
                    </div>
